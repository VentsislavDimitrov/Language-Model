...
class TrainModel:
    def __init__(self, gpt_model: GPTModel,
        ...
        """
        Initialize the training controller for the GPT model.

        Algorithm:
        ==========
        1. Store the GPT model reference for training and evaluation
        2. Set training parameters (epochs, data loaders, evaluation settings)
        3. Initialize AdamW optimizer with learning rate and weight decay
        4. Store evaluation configuration (frequency and batch iteration count)
        5. Store input text for sample generation (reasoning mode only)
        6. Store device (CPU/GPU) for tensor placement
        7. Store classification_mode to determine training strategy:
           - False: Language modeling/reasoning mode (generative)
           - True: Binary classification mode (discriminative)
        8. Initialize tracking arrays for losses, token counts, and accuracies
        9. Initialize counters for seen_tokens and global_step
        10. Instantiate Converter for text-token conversion operations

        Why This Design:
        ================
        - Single training class handles both reasoning and classification modes,
          reducing code duplication and maintenance overhead
        - classification_mode flag enables unified training loop with conditional
          behavior for loss computation, metrics, and evaluation
        - AdamW optimizer with weight decay provides adaptive learning rates and
          regularization to prevent overfitting during both training modes
        - Low learning rate (0.0001) ensures stable fine-tuning, especially
          important when adapting pretrained models to classification tasks
        - Weight decay (0.3) is set relatively high to encourage simpler models
          and reduce overfitting on smaller classification datasets
        - Separate tracking arrays for train/validation losses and accuracies
          enable comprehensive post-training analysis and visualization
        - seen_tokens tracking differs by mode:
          * Reasoning: counts individual tokens (batch_size * seq_length)
          * Classification: counts samples (batch_size)
        - evaluate_frequency controls computational cost vs. monitoring granularity
        - evaluate_iteration limits evaluation batches for speed while maintaining
          representative metrics
        - Converter instance enables seamless text↔token conversion for sample
          generation in reasoning mode
        - Storing input_text allows consistent sample generation across epochs
          for tracking qualitative progress

        Args:
            gpt_model: Initialized GPTModel instance with pretrained or random weights
            train_epoch: Number of complete passes through the training dataset
            train_loader: DataLoader providing training batches with input/target pairs
            validation_loader: DataLoader providing validation batches for evaluation
            evaluate_frequency: Number of training steps between evaluation runs
            evaluate_iteration: Number of batches to use when computing evaluation metrics
            input_text: Prompt text for sample generation (used only in reasoning mode)
            device: PyTorch device (cpu/cuda) for tensor placement
            classification_mode:
                - True: Model outputs 2-class logits for binary classification
                - False: Model outputs vocabulary logits for language modeling/generation
        """
    ...
    def train_model(self):
        ...
        1. For each training epoch:
           ...
           b. Iterate through each batch in train_loader:
              ...
              - Increment seen_tokens and global_step counters:
                * classification_mode=True: count samples (batch_size)
                * classification_mode=False: count tokens (batch_size * seq_length)
              - At evaluate_frequency intervals: evaluate on train and validation sets
              - Record losses and token counts
           c. After epoch completion:
              * classification_mode=True: compute classification accuracy on both datasets
              * classification_mode=False: generate sample text for qualitative inspection
           d. Log and store accuracy metrics (classification mode only)
        ...
        - classification_mode determines:
          * Loss computation strategy (per-token for reasoning, per-sample for classification)
          * Token counting method (numel() for reasoning, shape[0] for classification)
          * Evaluation metrics (accuracy for classification, text generation for reasoning)
        - Classification accuracy computed per epoch gives holistic view of model performance
        - Text generation provides qualitative insight into reasoning mode learning progress
        ...
        Returns:
            ...
            For classification_mode=False (reasoning mode):
                Tuple containing:
                - losses_from_train_loader: Array of training losses recorded during evaluation
                - losses_from_validation_loader: Array of validation losses recorded during evaluation
                - tracked_seen_tokens: Array of token counts at each evaluation point
        """
        for each_successful_epoch in tqdm(range(self.train_epoch), total=self.train_epoch):                              # The amount of iterations the training loop needs to be completed
    ...
    def evaluate_model(self) -> tuple[float, float]:
        ...
        5. Compute average loss on a subset of batches from each loader
           - classification_mode=True: computes classification loss (cross-entropy)
           - classification_mode=False: computes language modeling loss (per-token cross-entropy)
        ...
        - classification_mode is passed to CalculateLoss to adapt loss computation:
          * Reasoning mode: computes loss over all token positions (language modeling)
          * Classification mode: computes loss over sequence-level predictions
    ...
    def generate_and_print_sample(self):
        ...
        This method is only used in reasoning mode (classification_mode=False).
        ...
        - This method is called at the end of each epoch only in reasoning mode,
          providing regular qualitative checkpoints during training

        Note:
            This method is only active when classification_mode=False (reasoning mode).
            In classification mode, accuracy metrics are computed instead of text generation.
        """
        self.gpt_model.eval()
